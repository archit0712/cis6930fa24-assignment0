...
def extract_titles(parsed_data):
    """
    Extract only titles from the thorn-separated values.
    """
    titles = []
    for row in parsed_data:
        title = row.split('þ')[0]  # Split by 'þ' and take the first part (title)
        titles.append(title)
    return titles
# Test extracting titles from FBI API data
def test_extract_titles():
    parsed_data = parse_data(sample_data)
    result_titles = extract_titles(parsed_data)
    expected_titles = extract_titles(expected_output)
    
    assert expected_titles == result_titles
